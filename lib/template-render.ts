/**
 * Template-based renderer (legacy path).
 * Renders a card using a template definition with pre-defined slots,
 * palettes, and optional DALL-E art zone.
 */
...
function getFonts() {
  ...
  _fonts = [
    { name: 'Inter',    data: readFileSync(join(nm, 'inter', 'files', 'inter-latin-400-normal.woff')),    weight: 400 as const, style: 'normal' as const },
    { name: 'Inter',    data: readFileSync(join(nm, 'inter', 'files', 'inter-latin-700-normal.woff')),    weight: 700 as const, style: 'normal' as const },
    { name: 'Oswald',   data: readFileSync(join(nm, 'oswald', 'files', 'oswald-latin-700-normal.woff')),   weight: 700 as const, style: 'normal' as const },
    { name: 'Playfair Display', data: readFileSync(join(nm, 'playfair-display', 'files', 'playfair-display-latin-400-normal.woff')), weight: 400 as const, style: 'normal' as const },
    { name: 'Playfair Display', data: readFileSync(join(nm, 'playfair-display', 'files', 'playfair-display-latin-700-normal.woff')), weight: 700 as const, style: 'normal' as const },
...
export async function renderTemplateToBase64(
  ...
  for (const slot of template.slots) {
    ...
    // Auto-fit: char-count heuristic
    // REVIEW: uses char-count estimate, not true layout measurement
